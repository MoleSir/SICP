...
def subseqs(s):
    ...
    if not s:
        ...
        return without_first + insert_into_all(s[0], without_first)
...
def inc_subseqs(s):
    ...
    """
    def inc_subseq_helper(s, prev):
        if not s:
            return [[]]
        elif s[0] < prev:
            return inc_subseq_helper(s[1:], prev)
        else:
            a = inc_subseq_helper(s[1:], s[0])
            b = inc_subseq_helper(s[1:], -1)
            return insert_into_all(s[0], a) + b
    return inc_subseq_helper(s, -1)
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1:
        return 1
    return sum([num_trees(i + 1) * num_trees(n - i - 1) for i in range(n - 1)])
    ...
    def make_counter():
        ...
        def counter(order):
            ...
            if order == 'count':
                local_count += 1
                return local_count
            elif order == 'global-count':
                global_count += 1
                return global_count
            elif order == 'reset':
                local_count = 0
            elif order == 'global-reset':
                global_count = 0
        return counter
    return make_counter
